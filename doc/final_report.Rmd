...
*Figure 7: Each training accuracy was obtained using 10-fold cross-validation on the training data (with random state=10). You can see the accuracy variations across different max_depth settings of decision trees.*
...
knitr::kable(model_accuracy[2:3])
```
...
score <- as.character(round(rank[1,2:6], 5))
rank <- colnames(rank[2:6])
rank <- data.frame('Rank' = c(1, 2, 3, 4, 5), 'Factor' = rank, 'Gini Importance' = score)
