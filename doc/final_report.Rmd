...
*Figure 7: Each train accuracy was obtained using 10-fold cross-validation on the train data (with random state=10). We can see the accuracy varies across different max_depth setting of decision trees.*

From above, we can see that, the highest train accuracy was reached at the maximum depth of 4 for the decision tree. We then produced the following decision tree with setting the hyperparameter(max_depth) as 4 *(you may need to zoom in to read the details of the tree)*:
